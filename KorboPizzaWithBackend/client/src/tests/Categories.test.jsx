...
it("renders the categories data (unit)", () => {
    act(() => {
        render(categoriesRender(1, onClickCategory, fakeCategories), container);
    });
    ...
    act(() => {
        render(categoriesRender(3, onClickCategory, fakeCategories), container);
    });
...
it("renders the categories data (snapshot)", () => {
    ...
    let activeCategory = container.querySelector(".active");
    expect(pretty(activeCategory.textContent)).toMatchInlineSnapshot(`"Сырные"`);
    const categories = container.querySelector(".categories > ul").children;
    for (let i = 0; i < fakeCategories.length; i++) {
    ...
    act(() => {
        render(categoriesRender(5, onClickCategory, fakeCategories), container);
    });
    expect(
